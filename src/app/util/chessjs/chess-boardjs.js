    ...
    function constructor (containerElOrString, config) {
      ...
      function stopDraggedPiece (location) {
        ...
        if (isFunction(config.onDrop)) {
          ...
          if(draggedPieceSource === location) {
            if(!firstTappedSquare || firstTappedSquare.trim().length == 0) {
              firstTappedSquare = location;
              if(config.onFirstTap) {
                config.onFirstTap(firstTappedSquare)
              }
            } else {
              if(firstTappedSquare != location) {
                if(config.enableDoubleTap && config.onSecondTap) {
                  config.onSecondTap(firstTappedSquare, location);
                  firstTappedSquare = null;
                }
              }
            }
          } else {
      ...
      function mousedownSquare (evt) {
        ...
        if (!currentPosition.hasOwnProperty(square)) {
          if(firstTappedSquare != square) {
            if(config.enableDoubleTap && config.onSecondTap) {
              config.onSecondTap(firstTappedSquare, square);
              firstTappedSquare = null;
            }
          }
        }
