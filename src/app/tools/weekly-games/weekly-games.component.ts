...
export class WeeklyGamesComponent implements OnInit {
  ...
  moveFetchedForFen = this.initialPosition;
  checkingNovalty = false;
  ...
  onMove(fen : string) {
    ...
    this.store.dispatch(new GameAction.FetchMovesFromPositionFromWeeklyGame(fen));
    ...
    this.store.select(GameSelector.selectState).pipe(takeUntil(this.ngUnsubscribe)).subscribe(state=> {
      if(state.error) {
        ...
        if(state.isLoading)
          return
        let moves= state.weeklyGameMoves;
        if(moves) {
          ...
          tmpMove.sort(function(a : MoveDetails, b: MoveDetails) {
            return b.count - a.count;
          })
          this.service.getMovesOfPosition(this.currentFen).subscribe(res=> {
            this.checkingNovalty = true;
            if(res && res.body) {
              let dbMoves = res.body;
              let novalty = {}
              tmpMove.forEach(m=> {
                if(!dbMoves[m.move] || dbMoves[m.move]["count"] == m.count || dbMoves[m.move]["count"] == 2) {
                  m.novalty = true;
                  novalty[m.move] = true;
                }
              })
              this.dbMoves = []
              Object.keys(dbMoves).forEach(move => {
                if(dbMoves[move]["count"] > 0) {
                  if(novalty[dbMoves[move].move]) {
                    this.dbMoves.push({
                      ...dbMoves[move],
                      novalty : true
                    })
                  ...
                  this.gameCount+=dbMoves[move].count;
                }

              })
              this.dbMoves.sort(function(a : MoveDetails, b: MoveDetails) {
                return b.count - a.count;
              })
            }
            this.moves = tmpMove
            this.checkingNovalty = false;
          })
        }
